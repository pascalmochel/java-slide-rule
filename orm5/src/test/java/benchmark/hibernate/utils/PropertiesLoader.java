...
public class PropertiesLoader {
	...
	public static Properties load(final String propertiesBaseName, final Locale locale) {
		try {
			...
			try {
				...
				throw new OrmException("not found for locale=" + locale.toString()
	...
	protected static Properties load2(final String propertiesFileName) {
		...
		final InputStream is = Thread.currentThread().getContextClassLoader().getResourceAsStream(
				propertiesFileName);
		if (is == null) {
			throw new OrmException("could not open configuration file: " + propertiesFileName);
		}
		try {
			return load(is);
		} catch (final IOException e) {
			throw new OrmException("could not read configuration from: " + propertiesFileName, e);
		}
	}
